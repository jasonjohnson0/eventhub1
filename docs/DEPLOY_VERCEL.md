...
| `CRON_SECRET` | yes, for reminders to actually send | **Secret.** Gates `/api/cron/email-reminders` (spec 07): Vercel automatically sends `Authorization: Bearer $CRON_SECRET` on requests it makes to the `crons` entries in `vercel.json`, and the route checks that header. Without this set, the route 500s on every request rather than silently no-op'ing, so a missing reminder drain is loud, not quiet. **Correction, found the hard way on 2026-09-19:** the schedule was originally `*/15 * * * *` (every 15 minutes) with a note here that Hobby "only runs cron jobs once a day" — that undersold it. A Hobby-plan project doesn't just throttle a too-frequent cron down to daily, it **refuses to build any deployment at all** while `vercel.json` declares one (`cron_jobs_limits_reached`, surfaced by the deployment API, not shown anywhere in the dashboard UI) — this silently blocked every production deployment on this project for five days before the actual error was found. `vercel.json` now schedules it once daily (`0 13 * * *`) so deployments work on Hobby; switch back to `*/15 * * * *` only alongside an upgrade to Pro. |
